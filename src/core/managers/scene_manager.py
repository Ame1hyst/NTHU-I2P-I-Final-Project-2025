...
class SceneManager:
    ...
    def __init__(self):
        ...
        self._scenes = {}
        self.next_scene_name = None # Battle reset
    ...
    def change_scene(self, scene_name: str) -> None:
        if scene_name in self._scenes:
            ...
            self.next_scene_name = self._next_scene = scene_name
        else:
    ...
    def _perform_scene_switch(self) -> None:
        if self._next_scene is None:
            return
        # Screen shot for overlay
        if self._current_scene:
            self.previous_screen = self._current_scene
            self.previous_screen_name = self._current_scene.name
            self.previous_screen_surf = pg.display.get_surface().copy()
