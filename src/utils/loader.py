...
def load_img(path: str, size:int = None) -> pg.Surface:
    Logger.info(f"Loading image: {path}")
    img = pg.image.load(str(ASSETS_DIR / "images" / path)) if size is None else pg.transform.scale(pg.image.load(str(ASSETS_DIR / "images" / path)), size)
